...
class MarketService:
    ...
    async def get_market_stocks_data(
        self, search: str = "", sort_by: str = "name", sort_order: str = "asc", page: int = 1, page_size: int = 50
    ) -> MarketStocksData:
        all_stocks = await self.get_cached_stocks()
        filtered_stocks = self._filter_stocks(all_stocks, search)
        sorted_stocks = self._sort_stocks(filtered_stocks, sort_by, sort_order)
    ...
    def _filter_stocks(self, stocks: List[Dict[str, Any]], search: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for stock in stocks
            if (
                search_term in stock.get('name', '').lower()
                or search_term in stock.get('ticker', '').lower()
                or search_term in stock.get('full_name', '').lower()
                or search_term in str(stock.get('isin', '')).lower()
            )
        ]
    ...
    async def refresh_cache(self) -> Dict[str, Any]:
        try:
            ...
            return {"success": False, "message": f"Error refreshing cache: {str(e)}"}
